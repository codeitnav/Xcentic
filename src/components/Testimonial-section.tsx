...
export default function TestimonialSection() {
  ...
  return (
    <section id="testimonials" className="py-12 px-4 md:px-6 lg:px-8 relative overflow-hidden bg-gray-50">
      <div className="max-w-6xl mx-auto">
        {/* Floating Elements */}
        <div className="absolute inset-0 overflow-hidden">
          {/* You already have beautiful geometric elements */}
          {/* ... (same animated divs using mousePosition) */}
        </div>
            ...
            >
              <div className="flex flex-col items-center text-center">
                <div className="relative w-16 h-16 rounded-full overflow-hidden mb-4 mx-auto">
                  <Image
                    src={currentTestimonial.image || "/placeholder.svg?height=64&width=64"}
                ...
                <div className="flex items-center mt-4 mb-6">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      ...
                      className={cn(
                        "w-5 h-5",
                        i < Math.floor(currentTestimonial.rating) ? "text-yellow-400 fill-yellow-400" : "text-gray-300",
                      )}
                    />
                  ))}
                  <span className="ml-2 text-gray-700 font-medium">{currentTestimonial.rating.toFixed(1)}</span>
                </div>
                <p className="text-gray-600 leading-relaxed">{currentTestimonial.quote}</p>
              </div>
            </div>
          </div>
